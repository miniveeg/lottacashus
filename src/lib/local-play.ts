 ...
 *
 * Single-currency mode: only Sweeps Coins (SC).
 * RTP: 96.5% on all house games, 94.5% on case battles.
 */

import { GAME_RTP } from "./games/rtp";
import { getKenoMultiplier, type KenoRisk } from "./games/keno";

// ── Wallet (localStorage) — SC only ────────────────────────────────────────
const SC_KEY = "lottacash:local:sweeps";
const START_SC = 1000;
...
/** Always returns SC balance. coinType arg kept for call-site compatibility. */
export function localBalance(_coinType?: string): number {
  return readNum(SC_KEY, START_SC);
}
function localDebit(_coinType: string, amount: number): boolean {
  const cur = localBalance();
  if (amount > cur) return false;
  writeNum(SC_KEY, cur - amount);
  return true;
}
function localCredit(_coinType: string, amount: number) {
  const cur = localBalance();
  writeNum(SC_KEY, cur + amount);
}
...
export function localPlay(name: string, body: Record<string, unknown>): Result {
  try {
    switch (name) {
      case "place-limbo-bet":
        return placeLimboBet(body);
      case "place-crash-bet":
        return placeCrashBet(body);
      case "cash-out-crash":
        return cashOutCrash(body);
      case "mines-game":
        return minesGame(body);
      case "place-keno-bet":
        return placeKenoBet(body);
      case "place-roulette-bet":
        return placeRouletteBet(body);
      case "blackjack-game":
        return blackjackGame(body);
      case "place-slots-bet":
        return placeSlotsBet(body);
      default:
        return { data: null, error: `"${name}" is not available in local play.` };
    }
  } catch (e) {
...
function placeLimboBet(body: Record<string, unknown>): Result {
  ...
  return {
    data: {
      ...
      target,
      resultMultiplier: point,
      won,
      payout,
      profit: payout - wager,
      nonce: randInt(999999),
      coinType,
    },
    error: null,
  };
...
function placeRouletteBet(body: Record<string, unknown>): Result {
  ...
  if (wins(betType, pocket) && !keepWin(36 / 37, GAME_RTP)) {
    const losing: number[] = [];
    for (let n = 0; n < 37; n++) if (!wins(betType, n)) losing.push(n);
  ...
  return {
    data: {
      ...
      betType,
      resultPocket: pocket,
      resultColor: colorOf(pocket) as "red" | "black" | "green",
      won,
      payout,
      profit: payout - wager,
      multiplier: payoutMap[betType],
      nonce: randInt(999999),
      coinType,
    },
    error: null,
...
function blackjackGame(body: Record<string, unknown>): Result {
  ...
  if (action === "start") {
    ...
    if (pv === 21 && dv === 21) {
      ...
      outcome = "push";
      payout = wager;
      localCredit(coinType, wager);
      hand.dealerRevealed = true;
    } else if (pv === 21) {
      status = "bj";
      outcome = "bj";
      hand.dealerRevealed = true;
  ...
  if (action === "hit") {
    ...
    if (v > 21) {
      ...
      return { data: mapBj(handId, h, "bust", "lose", undefined, true), error: null };
    }
    if (v === 21) {
...
function standBj(handId: string, h: PendingBlackjack): Result {
  ...
  while (handValue(h.dealer) < 17 || (handValue(h.dealer) === 17 && isSoftHand(h.dealer)))
    h.dealer.push(h.shoe.pop()!);
  const pv = handValue(h.player),
    dv = handValue(h.dealer);
  let status: string,
    outcome: string,
    payout: number | undefined;
  if (dv > 21 || pv > dv) {
    if (keepWin(0.993, GAME_RTP)) {
      status = "win";
      outcome = "win";
      payout = Math.round(h.wager * 2 * 100) / 100;
      localCredit(h.coinType, payout);
    } else {
      status = "lose";
      outcome = "lose";
    }
    ...
    outcome = "push";
    payout = h.wager;
    localCredit(h.coinType, h.wager);
  } else {
    status = "lose";
    outcome = "lose";
  }
...
function mapBj(
  ...
  const dealerCards = revealed ? h.dealer.map(cardToNum) : h.dealer.slice(0, 1).map(cardToNum);
  ...
  return {
    ...
    nonce: randInt(999999),
    coinType: h.coinType,
    wager: h.doubled ? h.wager / 2 : h.wager,
    totalWager: h.wager,
    doubled: h.doubled,
...
const SLOTS_PAYOUTS = [3, 5, 8, 15, 30, 80, 190];
...
function placeSlotsBet(body: Record<string, unknown>): Result {
  ...
  return {
    data: {
      reels,
      symbols: reels.map((i) => SLOTS_SYMBOLS[i]),
      won,
      multiplier: mult,
      payout,
      outBalance: localBalance(),
      gameId: uid(),
      nonce: randInt(999999),
      coinType,
    },
