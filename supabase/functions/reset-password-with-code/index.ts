...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (fetchError || !row) {
      return jsonResponse({ error: "No reset code found. Request a new code." }, 400);
    }
    ...
    if (userError || !userId) {
      return jsonResponse({ error: "No account found for this email." }, 400);
    }
    ...
    if (updateError) {
      ...
      return jsonResponse({ error: "Could not update password. Try again." }, 500);
    }
